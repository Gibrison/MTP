...
% %sig_x=sqrt(sum((M(:,1)-mx).*(M(:,1))/ro));
 ...
 %M(:,1)=(M(:,1)-sum(M(:,1))/ro)/sqrt(sum((M(:,1)-sum(M(:,1))/ro).^2)/ro);
% 
% %my=sum(M(:,2))/ro;
...
for i=1:20
   ...
   % len1(end)
    ...
    for j=1:len2(end)
        ...
        % localise normalization
        
        M(s1-s2+1:s1,1)=(M(s1-s2+1:s1,1)-sum(M(s1-s2+1:s1,1))/ro)/sqrt(sum((M(s1-s2+1:s1,1)-sum(M(s1-s2+1:s1,1))/ro).^2)/ro);
        M(s1-s2+1:s1,2)=(M(s1-s2+1:s1,2)-sum(M(s1-s2+1:s1,2))/ro)/sqrt(sum((M(s1-s2+1:s1,2)-sum(M(s1-s2+1:s1,2))/ro).^2)/ro);
...
pt_ftr20(ends,:)=[];
%clear sin_t sin_p cos_t cos_p ends s1 s2 dist co temp
%=========================================================================
...
GMModel2 = gmdistribution.fit(Tr,k,'Regularize',0.0002,'Options',opt); %'CovType','diagonal',
...
for i=1:20
    ...
    for m=1:k
     ...
     for t=l(1):l(end)      %length(find(TrainD_20(:,6)==i))
        mu_i(:,m)=mu_i(:,m)+ (mvnpdf(Tr(t,:),u(m,:),(cov_m(:,:,m)))*Tr(t,:)*w(m))';      % column format
        sig_i(:,:,m)=sig_i(:,:,m)+ (mvnpdf(Tr(t,:),u(m,:),(cov_m(:,:,m)))*(Tr(t,:)'*Tr(t,:))*w(m));   % row format
    ...
    % sig_i(:,m)=sig_i(:,m)/norm1;
     ...
     sig_cap(:,:,m)=(1-alpha)*(cov_m(:,:,m)+u(m,:)'*u(m,:))+(alpha)*sig_i(:,:,m)/norm1-(mu_cap(:,m)*mu_cap(:,m)'); %var
     w1(m)=(1-alpha)*w(m);
     w2(m)=(alpha)*norm1;
    end
...
% GMMo.NumVariables=[5];
% GMMo.DistributionName='gmm';
% GMMo.NumComponents=50;
% GMMo.ComponentProportion=[wt(1,:)];
% GMMo.SharedCovariance=[false];
% GMMo.CovarianceType='diagonal';
% GMMo.mu=mean(1:5,:);
% GMMo.Sigma=covar{1};
% 
% %GMMo.Converged=;
% [idx,nlogl,P] = cluster(GMModel2,Tr(1:100,1:5));
%========================================================================
...
clear c co cos_t cos_p sin_t sin_p ends i j l1 l4 l5 l8 len1 len2 len3 m s1 s2 ro sig_i temp dist 
